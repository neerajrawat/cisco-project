...
def get_args(opts):
	...
	for opt, arg in opts:
		if opt == '-h':
			...
			print '6. Restore_Topology/RT/rt : With the use of this command we can reset the data and run whole program from the beginning. Note: This command is required to run only when command 5 has been executed on the Ring, else not required'
			print '7. Quit/Q/q : Quit the program'
...
def show_optimal_ring():
	...
	for ring in rings_list:
		if max_nodes == len(ring):
			...
			gui.Network_graph.drawTopology(ring_object,ring,'optimal_ring'+str(i)+'.png')
...
def errhandler(command):
	if command == "sra" or command == "srm":
		...
		elif Topo_Flag and not Ring_Flag and not Optimal_Flag:
			print "Warning : Before running rror command, run show ring and then show optimal ring command"
		elif Topo_Flag and Ring_Flag and not Optimal_Flag:
...
while i<len(lines):
	try:
		...
		print "Exception thrown in cdp read"
...
gui.Network_graph.drawTopology(topology,mt.Network_topology.EdgeList,'graph.png')
...
for ring in rings_list:
	...
	ring_object,isMultigraph=gui.Network_graph.makeGraph(ring) 
	ring_nx.append(ring_object)
	gui.Network_graph.drawTopology(ring_object,ring,'ring'+str(i)+'.png')
...
while True:
	...
	elif inputs == "clearline" or inputs == "clr" or inputs == "clear":
		Take_Action.get(inputs,errhandler)()
